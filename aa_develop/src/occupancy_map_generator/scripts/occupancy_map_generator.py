...
class GridMappingROS:
    def __init__(self):
        rospy.init_node('occupancyGridGenerator', anonymous=True)
        ...
        self.robot_frame = rospy.get_param('~robot_frame', 'state_estimation')
        ...
        self.laser_sub = rospy.Subscriber("/registered_scan", PointCloud2, self.pointcloud_callback, queue_size=2)
        self.map_pub = rospy.Publisher('/map', OccupancyGrid, queue_size=2)
        self.tf_sub = tf.TransformListener(rospy.Duration(15))
    ...
    def pointcloud_callback(self, data):
        try:
            ...
            # Set min/max angle and resolution for gridmapping initialization
            if not self.is_gridmapping_initialized:
                if angles:
                    laser_min_angle = min(angles)
                    laser_max_angle = max(angles)
                    if len(angles) > 1:
                        laser_resolution = (laser_max_angle - laser_min_angle) / (len(angles) - 1)
                    else:
                        laser_resolution = 0.01
                    laser_max_dist = max(ranges)
                    self.init_gridmapping(laser_min_angle, laser_max_angle, laser_resolution, laser_max_dist)
                else:
                    return

            # Only update if robot moved enough
            # if ((x - self.prev_robot_x) ** 2 + (y - self.prev_robot_y) ** 2 >= self.update_movement ** 2):
            # Sort by angle for consistency
            sorted_indices = np.argsort(angles)
            sorted_ranges = np.array(ranges)[sorted_indices]
            # sorted_angles = np.array(angles)[sorted_indices]

            # Update map
            gridmap = self.gridmapping.update(x, y, theta, sorted_ranges).flatten()
            self.prev_robot_x = x
            self.prev_robot_y = y

            # Publish map at specified frequency
